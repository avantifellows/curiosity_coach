...
STAGING_DB = get_db_config('STAGING')
...
def restore_staging_db(dump_file):
    """Restores the dump file to the staging database."""
    env = os.environ.copy()
    env['PGPASSWORD'] = STAGING_DB['password']
    ...
    print("Terminating existing connections to staging database...")
    try:
        subprocess.run(
            ['psql', *common_args, '-d', 'postgres', '-c', 
             f"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '{STAGING_DB['dbname']}' AND pid <> pg_backend_pid();"],
    ...
    print("Dropping staging database...")
    try:
        subprocess.run(['dropdb', *common_args, STAGING_DB['dbname']], env=env, check=True, capture_output=True, text=True)
    except subprocess.CalledProcessError as e:
    ...
    try:
        subprocess.run(['createdb', *common_args, STAGING_DB['dbname']], env=env, check=True, capture_output=True, text=True)
    except subprocess.CalledProcessError as e:
    ...
    # Restore the backup
    print(f"Restoring backup to '{STAGING_DB['dbname']}'...")
    try:
        subprocess.run(
            ['psql', *common_args, '-d', STAGING_DB['dbname'], '-f', dump_file],
            env=env, check=True, capture_output=True, text=True
        )
...
def main():
    ...
    try:
        ...
        restore_staging_db(dump_file)
        print("\nDatabase sync from production to staging completed successfully!")
