...
# Load environment variables from .env file in the parent directory (tests/)
...
def get_db_config(prefix):
    ...
    if not all(config.values()):
        print(f"Error: Missing database configuration for {prefix}. Please check your .env file.")
        sys.exit(1)
    return config
...
def create_backup():
    ...
    try:
        subprocess.run(dump_cmd, shell=True, check=True, capture_output=True, text=True)
...
def restore_local_db(dump_file):
    ...
    # Set PGPASSWORD for all subsequent psql/dropdb/createdb commands
    env = os.environ.copy()
    ...
    print("Terminating existing connections to local test database...")
    try:
        # Connect to the default 'postgres' db to run the terminate command
        subprocess.run(
        ...
        # It's okay if this fails (e.g., if the DB doesn't exist yet)
        print(f"Could not terminate connections (this might be okay): {e.stderr}")

    # Drop the existing database
    print("Dropping local test database...")
    try:
    ...
    print("Creating fresh local test database...")
    try:
