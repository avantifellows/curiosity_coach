...
def create_backup():
    """Dumps the staging database to a .sql file."""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    dump_file = f'db_dump_staging_{timestamp}.sql'
    
    # Create pg_dump command, using the password from the environment
    dump_cmd = (
        f"PGPASSWORD='{STAGING_DB['password']}' pg_dump "
        f"-h {STAGING_DB['host']} -U {STAGING_DB['user']} -d {STAGING_DB['dbname']} "
        f"-p {STAGING_DB['port']} -f {dump_file} --clean --no-owner --no-acl"
    )
    
    print("Creating backup from staging database...")
    try:
...
def main():
    ...
    try:
        ...
        print("\nDatabase sync from staging to local completed successfully!")
